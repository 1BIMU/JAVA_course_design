...
public class GroupMessageHandler implements MessageHandler {
     ...
     */
    public GroupMessageHandler(ServerController controller, ChatServer server, ServerWindow serverWindow, String currentUser) {
    ...
    public boolean handle(encap_info message, Socket socket, encap_info response) throws IOException {
        // 调用控制器处理群组消息
        controller.Group_handler(message, response);
        // 群组消息处理后继续处理后续消息
        return true;
    }
